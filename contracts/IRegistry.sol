...
interface IRegistry {
    ...
    function getRefunderCountFor(address targetAddress, bytes4 interfaceId) external view returns(uint256);
    function getRefunderForAtIndex(address targetAddress, bytes4 interfaceId, uint256 index) external view returns(address);
}